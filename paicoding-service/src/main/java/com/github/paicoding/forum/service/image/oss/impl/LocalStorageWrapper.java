 ...
 */
@Slf4j
@ConditionalOnExpression(value = "#{'local'.equals(environment.getProperty('image.oss.type'))}") // 条件配置： 只有在配置文件中的image.oss.type属性为local时才会实例化该类
@Component
public class LocalStorageWrapper implements ImageUploader {
    ...
    @Override
    public String upload(InputStream input, String fileType) { // 用于将给定的输入流保存在本地的文件系统
        // 记录耗时分布
        ...
        try {
            ...
            // 这里调用了FileWriteUtil.saveFileByStream， 方法来对图片进行保存
